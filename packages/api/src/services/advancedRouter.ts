...
export class AdvancedRouterService {
  private provider: JsonRpcProvider;
  private tjRouter: Contract;
  private pangolinRouter: Contract;

  constructor() {
    const rpcUrl = process.env.AVALANCHE_RPC_URL || 'https://api.avax.network/ext/bc/C/rpc';
    this.provider = new JsonRpcProvider(rpcUrl);
    
    this.tjRouter = new Contract(DEX_CONFIG.traderJoe.router, TRADER_JOE_ABI, this.provider);
    this.pangolinRouter = new Contract(DEX_CONFIG.pangolin.router, PANGOLIN_ABI, this.provider);
  }
  ...
  async findOptimalRoute(
    ...
    logger.info(`Finding optimal route: ${tokenIn} -> ${tokenOut}, amount: ${amountIn}`);
    ...
    if (allowSplit && allRoutes.length >= 2) {
      ...
      if (splitAdvantage > 0.001) {
        logger.info(`Split routing is better: +${(splitAdvantage * 100).toFixed(2)}%`);
        return {
  ...
  private async findDirectRoutes(
    ...
    try {
      ...
      logger.debug('TraderJoe direct route failed', error);
    }
    ...
    try {
      ...
      logger.debug('Pangolin direct route failed', error);
    }
   ...
   */
  private async calculateSplitRoute(
    routes: Route[],
    totalAmountIn: bigint
  ): Promise<{ routes: Route[]; totalAmountOut: bigint; totalGasEstimate: number }> {
    // Take top 2 routes
   ...
   */
  private async getQuoteForRoute(route: Route, amountIn: bigint): Promise<bigint> {
   ...
   * Get Pangolin quote
   */
  private async getPangolinQuote(
    tokenIn: string,
    tokenOut: string,
    amountIn: bigint
  ): Promise<bigint> {
   ...
   */
  async calculateGasCost(gasUnits: number): Promise<bigint> {
    try {
...
// Singleton instance
export const advancedRouter = new AdvancedRouterService();