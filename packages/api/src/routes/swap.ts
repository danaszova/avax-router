...
import { getTokenAddress, TOKENS } from '../services/dex-apis';
 ...
 */
SwapRouter.get('/prepare', async (req: Request, res: Response) => {
  try {
    ...
    const amountInWei = ethers.parseUnits(amountIn as string, decimalsIn);
    ...
    const approveData = new ethers.Contract(tokenInAddress, ERC20_ABI, provider)
    ...
    res.json({
      ...
      quote: {
        ...
        expectedOutput: ethers.formatUnits(bestRoute.amountOut, decimalsOut),
        minimumOutput: ethers.formatUnits(minAmountOut, decimalsOut),
    ...
    logger.error('Prepare swap error:', error);
    res.status(500).json({
 ...
 */
SwapRouter.get('/approve-check', async (req: Request, res: Response) => {
  try {
    ...
    const provider = new ethers.JsonRpcProvider('https://api.avax.network/ext/bc/C/rpc');
    const tokenContract = new ethers.Contract(tokenIn as string, ERC20_ABI, provider);

    const allowance = await tokenContract.allowiance(userAddress as string, DEX_ROUTER_ADDRESS);
    const amountInBN = BigInt(amountIn as string);
    ...
    res.json({
      ...
      requiredAmount: amountIn as string,
      tokenAddress: tokenIn,
      spenderAddress: DEX_ROUTER_ADDRESS,
    });
  } catch (error: any) {
    logger.error('Approve check error:', error);
    res.status(500).json({
 ...
 */
SwapRouter.get('/tx/:type', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get tx error:', error);
    res.status(500).json({
